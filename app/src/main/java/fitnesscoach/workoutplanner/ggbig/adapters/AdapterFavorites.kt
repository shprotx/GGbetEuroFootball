...
import fitnesscoach.workoutplanner.ggbig.databinding.ItemButtonsBinding
...
class AdapterFavorites(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            fitnesscoach.workoutplanner.ggbig.adapters.AdapterFavorites.Companion.VIEW_TYPE_FIXED -> {
                val view = LayoutInflater.from(parent.context)
                    .inflate(R.layout.item_buttons, parent, false)
                binding = ItemButtonsBinding.bind(view)
                FixedViewHolder(binding)
            }
            fitnesscoach.workoutplanner.ggbig.adapters.AdapterFavorites.Companion.VIEW_TYPE_NORMAL -> {
                val view = LayoutInflater.from(parent.context)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder is fitnesscoach.workoutplanner.ggbig.adapters.AdapterFavorites.MatchViewHolder) {
            val dataItem = response[position - 1]
            holder.bind(dataItem)
        } else if (holder is fitnesscoach.workoutplanner.ggbig.adapters.AdapterFavorites.FixedViewHolder)
            holder.bind()
    }

    override fun getItemViewType(position: Int): Int {
        return if (position == 0) fitnesscoach.workoutplanner.ggbig.adapters.AdapterFavorites.Companion.VIEW_TYPE_FIXED
        else fitnesscoach.workoutplanner.ggbig.adapters.AdapterFavorites.Companion.VIEW_TYPE_NORMAL
    }
